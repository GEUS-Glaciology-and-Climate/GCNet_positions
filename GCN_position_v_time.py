...
meta_2014=pd.read_excel('./meta/GC-Net locations and numbers 2014 modified.xlsx')
print(meta_2014.columns)

meta=pd.read_csv('/Users/jason/Dropbox/AWS/GCNET/ancillary/GC-Net_info_incl_1999.csv')
print(meta.columns)

for st,name in enumerate(meta.name):
    v_meta=np.nan
    v_meta=np.where(name==meta_2014.nickname)
    if len(v_meta[0])>0:v_meta=v_meta[0][0]
    print(st,name,meta.elev[st],meta_2014.elev[v_meta])
#%%
